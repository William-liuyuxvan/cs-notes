...
遇到问题：在用户进行抢购优惠券时，订单会保存到表中，而订单如果用数据库自增ID的方法会出现问题：

- id的规律性太明显
- 受单表数据量的限制

解决方法：全局ID生成器，是一种在分布式系统下用来生成全局唯一ID的工具，一般满足下面的特性：
...
为了增加ID的安全性，我们可以不直接使用Redis自增的数值，而是通过拼接完成：

![image-20250611221131870](Redis.assets/image-20250611221131870.png)
...
- 符号位占用一位，不太影响使用，已经可以达到69年。
- 直接显示保留符号位为0，这样可以避免运算错误或者数值溢出意外生成负数ID。
